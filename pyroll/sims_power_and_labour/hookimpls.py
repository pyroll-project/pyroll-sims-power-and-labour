...
def equivalent_height_at_neutral_line(roll_pass: RollPass):
    return roll_pass.out_profile.equivalent_rectangle.height + 2 * roll_pass.roll.nominal_radius * (1 - np.cos(roll_pass.equivalent_neutral_line_angle))
...
def sims_force_function(roll_pass: RollPass):
    ...
    reccuring_part_c = roll_pass.roll.nominal_radius / roll_pass.out_profile.equivalent_rectangle.height
...
def roll_force(roll_pass: RollPass):
    ...
    return mean_flow_stress * np.sqrt(roll_pass.roll.nominal_radius * roll_pass.equivalent_height_change) * roll_pass.sims_force_function


@RollPass.hookimpl
def sims_torque_function(roll_pass: RollPass):
    entry_angle = np.arcsin(roll_pass.roll.contact_length / roll_pass.roll.nominal_radius)
...
def roll_torque(roll_pass: RollPass, roll: RollPass.Roll):
    mean_flow_stress = (roll_pass.in_profile.flow_stress + 2 * roll_pass.out_profile.flow_stress) / 3
    return 2 * roll.nominal_radius ** 2 * mean_flow_stress * roll_pass.sims_torque_function
